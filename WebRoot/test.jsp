<%@ page language="java" import="java.util.*" import="net.sf.*" import="java.sql.*" contentType="text/html; charset=utf-8" %>
<%
...
try
{
rs=(ResultSet)stmt.executeQuery("select * from ftable where tittle=\'"+key+"\'");
...
%>
<tr bgcolor="#4F81BD"style="color: #fff;">
   <td id="ID<%=a %>"><%=rs.getString("id") %></td>   
  <td><%=rs.getString("usename")%></td>   
   <td> <%=rs.getString("account")%></td>   
  <td><%=rs.getString("tittle")%></td>
  <td class="biaoge"><input type="button" value="删除"  onclick="huoqu<%=a %>()"></td>
  <td class="biaoge"> <input type="button" value="详情" onclick="xiangqing<%=a %>()"></td>   
</tr>

<script>
function huoqu<%=a %>(){
if(window.confirm('你确定删除吗吗？')){
            
              }else{
                
                 return false;
             }
var b=document.getElementById("ID<%=a %>").valueOf().innerHTML;
